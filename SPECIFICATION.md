...
### 4.8 3Dボックスステージ（壁面・天井）

ブラウザ内を「3Dの箱」のように見せるため、バックスクリーン・側面壁・天井を配置します。カメラの視差効果（フェイストラッキング/マウスドラッグ）でカメラが動くと壁面が見え隠れし、奥行き感・立体感が強調されます。

**構成要素:**

| 要素 | ジオメトリ | サイズ計算 | 配置 | 向き |
|---|---|---|---|---|
| **バックスクリーン（背面壁）** | `PlaneGeometry` | `2 × D × tan(FOV/2)` で画面全体を覆う | カメラ正面奥（Z軸マイナス方向） | カメラに正対 |
| **左壁** | `PlaneGeometry` | 奥行き × 高さ | バックスクリーン左端 | Y軸90°回転 |
| **右壁** | `PlaneGeometry` | 奥行き × 高さ | バックスクリーン右端 | Y軸-90°回転 |
| **天井** | `PlaneGeometry` | 幅 × 奥行き | バックスクリーン上端 | X軸90°回転 |

**バックスクリーンのサイズ計算:**

正面から見たときに画面全体を覆うよう、以下の式でサイズを算出します：

```
D = カメラZ位置 - バックスクリーンZ位置
...
| 項目 | 詳細 |
|---|---|
| マテリアルタイプ | `MeshStandardMaterial` |
| 描画面 | `DoubleSide`（箱の内側から正しく描画） |
| テクスチャ | 単色・グラデーション・テクスチャ・動画から選択可能 |
| ライティング | シーンの既存ライティングが壁面に自然に反映 |

**パララックス効果との連携:**
- カメラが左右に動くと側面壁が見え隠れし、箱の中にいるような没入感を生む
- カメラが上下に動くと天井・床が見え隠れする
- ズーム操作でバックスクリーンとの距離感が変化
